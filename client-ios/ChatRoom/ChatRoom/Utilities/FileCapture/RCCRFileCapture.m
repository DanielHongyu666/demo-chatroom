...
@implementation RCCRFileCapture
{
...
//        errorBlock(error);
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self setupReaderOnError];
    });
}
- (void)setupReaderOnError {
...
- (void)readNextBuffer {
    ...
    if (_reader.status == AVAssetReaderStatusCompleted) {
        [_reader cancelReading];
        _reader = nil;
        if (self.delegate && [self.delegate respondsToSelector:@selector(didReadCompleted)]) {
