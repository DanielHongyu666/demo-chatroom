...
- (void) KeyboardWillShow:(NSNotification*)notification {
    ...
    if ([self.delegate respondsToSelector:@selector(onInputBarControlContentSizeChanged:withAnimationDuration:andAnimationCurve:)]) {
        [self.delegate onInputBarControlContentSizeChanged:keyboardBounds withAnimationDuration:0.5 andAnimationCurve:curve];
    }
//
...
- (void)didTouchEmojiButton:(UIButton *)sender {
    ...
    if (!sender.selected) {
        ...
        if (!(_emojiBoardView.frame.size.width > 0)) {
            ...
            [_emojiBoardView setFrame:CGRectMake(0, 0, self.bounds.size.width, ISX ? (HeightEmojBoardView + 34) : HeightEmojBoardView)];
        }
